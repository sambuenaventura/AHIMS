...
// Check if the nurse is authenticated and has submitted current medications for the current patient
if (auth()->check() && $currentMedication && $currentMedication->nurse_user_id === $nurseUserId) {
    // Nurse is authenticated and has submitted current medications for the patient
    // Set nurse_user_id to the authenticated user's ID
    $validatedCurrentMedication['nurse_user_id'] = $nurseUserId;

    // Retrieve the nurse's information
    $nurse = auth()->user(); // Use the authenticated user as the nurse
} elseif ($currentMedication && $currentMedication->nurse_user_id) {
    // Another nurse has submitted the current medications
    // Retrieve the nurse's information based on the nurse_user_id associated with the current medication
    $nurse = \App\Models\User::find($currentMedication->nurse_user_id);
} else {
    // If no nurse is authenticated or has submitted current medications, set nurse_user_id to null or some default value
    $validatedCurrentMedication['nurse_user_id'] = null; // Or any default value you want to set
    $nurse = null; // Set nurse information to null
}
...
// // Add the new code here for updating current medication
...
//     // Add validation for other current medication fields as needed
// ]);

// // Retrieve the authenticated user's ID (Nurse 1)
// $nurseUserId = auth()->user()->id;

// // Check if the patient has existing current medication
// if ($patient->currentMedication) {
//     // Nurse 1 is updating the current medication
//     $patient->currentMedication->update(array_merge($validatedCurrentMedication, ['nurse_user_id' => $nurseUserId]));
// } else {
//     // Nurse 1 is creating a new current medication record
//     $patient->currentMedication()->create(array_merge($validatedCurrentMedication, ['nurse_user_id' => $nurseUserId]));
// }
