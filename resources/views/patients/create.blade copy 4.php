...
<main class="bg-custom-901 mt-20">
    ...
    <section id="hero">
        ...
        <div class="left">
            ...
            <div class="left-5">
                <h3>V. Select room number</h3>

                <div class="hospital-number-picker grid grid-cols-3 gap-4">
                    @for ($i = 1; $i <= 20; $i++)
                        @php
                            $disabled = in_array($i, $assignedRooms) ? 'disabled' : '';
                            $bgColorClass = in_array($i, $assignedRooms) ? 'bg-custom-50' : 'bg-gray-200';
                        @endphp
                        <label for="hospital_number{{$i}}" class="block text-center cursor-pointer p-4 rounded-lg relative {{$bgColorClass}} {{$disabled}}">
