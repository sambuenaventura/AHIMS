        ...
        <div class="boxes">
            ...
            <div class="box box1 flex-col bg-custom-101 mt-10 shadow-md">
                ...
                <form action="{{ route('patients.updateDischargeDate', ['patient_id' => $patient->patient_id]) }}" method="POST" class="flex gap-4">
                    ...
                    <button type="submit" class="btn btn-success btn-custom-style btn-submit">Set</button>
                </form>
            </div>
        @endif
        ...
        <div class="card pe-0 mb-4 shadow-md">
            <div class="card-body m-2">
              <div class="d-flex justify-content-between mb-4">
                ...
                <a href="/nurse-patients/add-remark/{{$patient->patient_id}}" class="btn btn-success" type="submit">Add Remark</a>
                </div>  
                ...
                <div class="card-group mt-2 mb-2 nurse-remark">
                    @foreach ($patient->vitalSigns->take(4)->reverse() as $key => $vitalSign)
                    <!-- 'take(4)' limits the number of records to four, and 'reverse()' ensures the latest records are shown -->
                    <div class="card" style="position: relative;">
                        @if($key === 0)
                        <div class="badge bg-success" style="font-size: 0.65em; position: absolute; top: 0; right: 0; padding: 4px;">NEW</div>
                        @endif
                    ...
                    <div class="card-header" id="heading{{ $index }}" onclick="toggleCollapse({{ $index }})" data-toggle="collapse" data-target="#collapse{{ $index }}">
                        <div class="mb-0 d-flex justify-content-between align-items-center">
                            <p class="mb-0 d-flex justify-content-between align-items-center">
                                ...
                                @if ($loop->first)
                                <span class="ml-2 badge bg-success" style="font-size: 0.65em; padding: 4px;">NEW</span>
                                @endif
                            </p>
                    ...
                    <div id="collapse{{ $index }}" class="custom-collapse" aria-labelledby="heading{{ $index }}" data-parent="#medicationRemarksAccordion">
                        <div class="card-body">
                            ...
                            <div class="flex-col">
                                ...
                                <p>{{ $medicationRemark->medication ?? 'N/A' }}</p>
                            </div>
                            <div class="flex-col">
                                <p class="font-bold">Dosage</p>
                                <p>{{ $medicationRemark->medication_dosage ?? 'N/A' }}</p>
                            </div>
                            <div class="flex-col">
        ...
        <div class="card pe-0 mb-4 shadow-md">
            <div class="card-body m-2">
              <div class="d-flex justify-content-between mb-4">
                ...
                <a href="/nurse-patients/add-progress-note/{{$patient->patient_id}}" class="btn btn-success" type="submit">Add Progress Note</a>
            </div>  
            <div class="table-header">
                <hr>
                {{-- <p class="font-bold">Medications, Treatment & etc.</p> --}}
                    ...
                    <div id="progressCollapse{{ $index }}" class="custom-collapse" aria-labelledby="progressHeading{{ $index }}" data-parent="#progressNotesAccordion">
                        <div class="card-body">
                            <div class="flex-col">
                                ...
                                <p>{{ $progressNote->focus ?? 'N/A' }}</p>
                            </div>
                            <div class="flex-col">
                                <p class="font-bold">Progress Notes</p>
                                <p>{{ $progressNote->prog_notes ?? 'N/A' }}</p>
                            </div>
                        </div>
                        <div class="card-footer">
                            <div class="flex-row">
                            ...
                            <div class="flex-row">
                                <small class="text-muted">Date & Time:</small>
                                <small class="text-muted">{{ \Carbon\Carbon::parse($progressNote->created_at)->format('g:i A n/j/Y') }}</small>
                            </div>
                        </div>
                    </div>
                </div>
