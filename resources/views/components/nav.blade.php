...
<div class="hamburger-menu">
  ...
  <ul class="menu__box">
    ...
    <li class="nav-item">
      <a class="menu__item" href="{{ 
          auth()->user()->role === 'admission' ? '/admission-patients' :
          ...
          (auth()->user()->role === 'nurse' ? 'Laboratory Services' : 
    ...
    <li>
      <a class="menu__item" href="{{ auth()->user()->role === 'nurse' ? '/nurse/imaging-services' : 
      (auth()->user()->role === 'medtech' ? '/medtech-results' : 
      (auth()->user()->role === 'radtech' ? '/radtech-results' : '#')) 
      }}">
        {{ auth()->user()->role === 'nurse' ? 'Imaging Services' : 
        (auth()->user()->role === 'medtech' ? 'Results' : 
        (auth()->user()->role === 'radtech' ? 'Results' :
        'Fallback Text')) }}
      </a>
    </li>
    @endif
    ...
    <li>
      <a class="menu__item" href="{{ auth()->user()->role === 'admin' ? '/doctors-list' : '#' }}">

              {!! auth()->user()->role === 'admin' ? 'Physicians' : 'Fallback Text' !!}
      </a>
    </li>
    <li>
      <a class="menu__item" href="{{ auth()->user()->role === 'admin' ? '/admissions-list' : '#' }}">

              {!! auth()->user()->role === 'admin' ? 'Admissions' : 'Fallback Text' !!}
      </a>
    </li>
    <li>
      <a class="menu__item" href="{{ auth()->user()->role === 'admin' ? '/nurses-list' : '#' }}">

              {!! auth()->user()->role === 'admin' ? 'Nurses' : 'Fallback Text' !!}
      </a>
    </li>
    <li>
      <a class="menu__item" href="{{ auth()->user()->role === 'admin' ? '/medtechs-list' : '#' }}">

              {!! auth()->user()->role === 'admin' ? 'MedTechs' : 'Fallback Text' !!}
      </a>
    </li>
    <li>
      <a class="menu__item" href="{{ auth()->user()->role === 'admin' ? '/radtechs-list' : '#' }}">
    ...
    <li class="nav-item">
      <a href="/logout" class="menu__item" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
        Logout
      </a>
...
<nav class="bg-white p-4 flex justify-between">
  ...
  <div class="flex items-center">
    ...
    <a href="#" class="notif text-gray-800 hover:text-gray-600">
    <button class="notif btn btn-light"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="position-sticky">
          <ul class="nav flex-column">
            ...
            <li class="nav-item mt-5">
              <a class="btn p-2 text-dark bg-opacity-25 material-symbols-outlined text-white" href="#" disabled>
                  <img src="https://api.dicebear.com/7.x/thumbs/svg?seed={{ substr(auth()->user()->first_name, 0, 1) . substr(auth()->user()->last_name, 0, 1) }}" alt="Avatar" width="60" height="60" style="border-radius: 50%;">
              </a>
          </li>
      ...
      @endif
            ...
            <li class="nav-item my-5">
              <a href="/logout" class="btn bg-light p-2 text-dark bg-opacity-25 material-symbols-outlined text-white" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
                Logout
              </a>
              <p>Log out</p>
              <form id="logout-form" action="/logout" method="POST" style="display: none;">
