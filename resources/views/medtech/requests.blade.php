        ...
        <div class="card pe-0">
            <div class="card-body m-1">
                <div class="d-flex justify-content-between mb-4">
                    ...
                    <form action="{{ route('medtech.requests') }}" method="GET" class="mb-3">
                ...
                <ul class="nav nav-underline overflow-x-auto">
                    {{-- <li class="nav-item">
                        <a class="nav-link {{ request('status') === 'pending' ? 'active btn-submit text-white rounded px-3 py-2' : 'text-secondary' }}" href="{{ route('medtech.requests', ['status' => 'pending']) }}">Pending Requests</a>
                    </li> --}}
                    <li class="nav-item">
                        <a class="nav-link {{ request('status') === 'accepted' ? 'active btn-submit text-white rounded px-3 py-2' : 'text-secondary' }}" href="{{ route('medtech.requests', ['status' => 'accepted']) }}">Accepted Requests</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ request('status') === 'completed' ? 'active btn-submit text-white rounded px-3 py-2' : 'text-secondary' }}" href="{{ route('medtech.requests', ['status' => 'completed']) }}">Completed Requests</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ request('status') === 'declined' ? 'active btn-submit text-white rounded px-3 py-2' : 'text-secondary' }}" href="{{ route('medtech.requests', ['status' => 'declined']) }}">Declined Requests</a>
                    </li>
                            ...
                            <tr>
                               ...
                               <td style="min-width: 180px;">{{ $request->sender_message }}</td>
                                ...
                                <td style="text-align: center;">
                                    <div class="">
                                      <a href="/medtech-patients/{{ $request->patient_id }}/requests/{{ $request->request_id }}" class="badge rounded-pill text-bg-success d-inline-flex align-items-center gap-0.5" style="font-size: 1em;">
